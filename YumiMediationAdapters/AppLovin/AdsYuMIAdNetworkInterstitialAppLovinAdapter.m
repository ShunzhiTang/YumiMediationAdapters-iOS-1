...
@interface AdsYuMIAdNetworkInterstitialAppLovinAdapter () <ALAdLoadDelegate, ALAdDisplayDelegate,
...
- (void)timeOutTimer {
    ...
    [self stopTimer];
    [self adapter:self
        didInterstitialFailAd:[AdsYuMIError errorWithCode:AdYuMIRequestTimeOut description:@"Applovin time out"]];
}

- (void)preasentInterstitial {
    [self.interstitialAd showOver:[UIApplication sharedApplication].keyWindow.rootViewController.view.window
                        andRender:self.ad];
}
...
- (void)adService:(alnonnull ALAdService *)adService didFailToLoadAdWithError:(int)code {
    ...
    [self stopTimer];
    [self adapter:self
        didInterstitialFailAd:[AdsYuMIError errorWithCode:AdYuMIRequestNotAd description:@"Applovin no ad"]];
}
...
- (void)videoPlaybackEndedInAd:(alnonnull ALAd *)ad
             atPlaybackPercent:(alnonnull NSNumber *)percentPlayed
