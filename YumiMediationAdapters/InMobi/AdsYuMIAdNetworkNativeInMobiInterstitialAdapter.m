...
#import <YumiMediationSDK/AdsYuMIKit/AdsYuMiInterstitialNativeViewController.h>
#import <InMobiSDK/InMobiSDK.h>

@interface AdsYuMIAdNetworkNativeInMobiInterstitialAdapter ()<AdsYuMiInterstitialNativeViewControllerDelegate,IMNativeDelegate>
{
...
-(void)nativeDidFinishLoading:(IMNative*)native{
  ...
  if (!native  || !native.adContent) {
    [self adapter:self didInterstitialFailAd:[AdsYuMIError errorWithCode:AdYuMIRequestNotAd description:@"Inmobi no ad"]];
    return;
  }
  ...
  if (imobeDict==nil||![imobeDict objectForKey:@"screenshots"]) {
    [_adsYuMIInMobiSelf adapter:self didInterstitialFailAd:[AdsYuMIError errorWithCode:AdYuMIRequestNotAd description:@"Inmobi no ad"]];
    return;
  }
  ...
  _gdtInterstitialWebView = [[AdsYuMiInterstitialNativeViewController alloc]initWithCustomView:CGRectMake(0, 0,width, height) delegate:self];
  ...
  NSString * interstitialStr =[[NSString alloc]initWithData:interstitialData encoding:NSUTF8StringEncoding];
   interstitialStr=[NSString stringWithFormat:interstitialStr,@"100%",@"100%",@"100%",@"100%",[imobeDict objectForKey:@"landingURL"],url];
...
{
  NSBundle *bundle = [NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:@"YMAutoNomousRes" ofType:@"bundle"]];
...
-(void)timeOutTimer{
  ...
  isReading=YES;
  [self stopTimer];
  [_adsYuMIInMobiSelf adapter:self didInterstitialFailAd:[AdsYuMIError errorWithCode:AdYuMIRequestTimeOut description:@"Inmobi time out"]];
}
/**
 *  插屏展示
 */
