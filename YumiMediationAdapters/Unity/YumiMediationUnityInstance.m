...
- (NSString *)adapterKey:(NSString *)placementId {
    ...
    [self.adaptersDict.allKeys enumerateObjectsUsingBlock:^(NSString * _Nonnull key, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)delegateErrorIfNeed:(BOOL)isLoadFail  errorMsg:(NSString *)errorMsg{
    ...
    [self.adaptersDict enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, id<YumiMediationCoreAdapter>  _Nonnull coreAdapter, BOOL * _Nonnull stop) {
        ...
        if (isLoadFail) {
            if (adType == YumiMediationAdTypeInterstitial) {
                ...
                [((YumiMediationInterstitialAdapterUnity *)coreAdapter).delegate coreAdapter:coreAdapter coreAd:nil didFailToLoad:errorMsg adType:adType];
            }
            if (adType == YumiMediationAdTypeVideo) {
                
                [((YumiMediationVideoAdapterUnity *)coreAdapter).delegate coreAdapter:coreAdapter coreAd:nil didFailToLoad:errorMsg adType:adType];
            }
            return;
        }
        // fail to show
        if (adType == YumiMediationAdTypeInterstitial) {
            ...
            [((YumiMediationInterstitialAdapterUnity *)coreAdapter).delegate coreAdapter:coreAdapter failedToShowAd:nil errorString:errorMsg adType:adType];
        }
        if (adType == YumiMediationAdTypeVideo) {
            
            [((YumiMediationVideoAdapterUnity *)coreAdapter).delegate coreAdapter:coreAdapter failedToShowAd:nil errorString:errorMsg adType:adType];
        }
...
- (void)unityAdsDidStart:(NSString *)placementId {
    ...
    if (adType == YumiMediationAdTypeInterstitial) {
        [((YumiMediationInterstitialAdapterUnity *)adapter).delegate coreAdapter:adapter didOpenCoreAd:nil adType:YumiMediationAdTypeInterstitial];
         [((YumiMediationInterstitialAdapterUnity *)adapter).delegate coreAdapter:adapter didStartPlayingAd:nil adType:YumiMediationAdTypeInterstitial];
        return;
    }
    
    if (adType == YumiMediationAdTypeVideo) {
        [((YumiMediationVideoAdapterUnity *)adapter).delegate coreAdapter:adapter didOpenCoreAd:nil adType:YumiMediationAdTypeVideo];
        [((YumiMediationVideoAdapterUnity *)adapter).delegate coreAdapter:adapter didStartPlayingAd:nil adType:YumiMediationAdTypeVideo];
        return;
    }
}
...
- (void)unityAdsDidFinish:(NSString *)placementId withFinishState:(UnityAdsFinishState)state {
    ...
    if (adType == YumiMediationAdTypeInterstitial) {
        [((YumiMediationInterstitialAdapterUnity *)adapter).delegate coreAdapter:adapter didCloseCoreAd:adapter isCompletePlaying:NO adType:YumiMediationAdTypeInterstitial];
        return;
    }
...
- (void)unityAdsDidClick:(NSString *)placementId{
    ...
    if (adType == YumiMediationAdTypeInterstitial) {
        [((YumiMediationInterstitialAdapterUnity *)adapter).delegate coreAdapter:adapter didClickCoreAd:nil adType:YumiMediationAdTypeInterstitial];
        return;
    }
    if (adType == YumiMediationAdTypeVideo) {
        [((YumiMediationVideoAdapterUnity *)adapter).delegate coreAdapter:adapter didClickCoreAd:nil adType:YumiMediationAdTypeVideo];
        return;
    }
}
