...
-(UIImage *)getBundleResourcesFromCustomBundle:(NSString *)name type:(NSString *)type{
    ...
    NSString *resourcesPath = [YumiMediationFacebook pathForResource:[NSString stringWithFormat:@"%@%@",name,@"@2x"] ofType:type];
...
-(YumiFacebookAdapterInterstitialVc *)createInterstitialVc{
  interstitial = [self getNibResourceFromCustomBundle:@"YumiFacebookInterstitialNativeAdapter" type:@"xib"] ;
   ...
   [interstitial.closeButton addTarget:self action:@selector(closeFacebookIntestitial) forControlEvents:UIControlEventTouchUpInside];
    return interstitial;
}
