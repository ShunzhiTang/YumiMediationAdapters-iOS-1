...
-(void)getAd{
  ...
  _gdtInterstitialWebView = [[AdsYuMiInterstitialNativeViewController alloc]initWithCustomView:[UIScreen mainScreen].bounds delegate:self isBorder:NO];
...
-(void)nativeAdSuccessToLoad:(NSArray *)nativeAdDataArray {
  ...
  if(!nativeAdDataArray || ![nativeAdDataArray objectAtIndex:0]){
    [self adapter:self didInterstitialFailAd:[AdsYuMIError errorWithCode:AdYuMIRequestNotAd description:@"GDT no ad"]];
    return;
  }
  ...
  if (!_currentAd.properties || ![_currentAd.properties objectForKey:GDTNativeAdDataKeyImgUrl] || ![_currentAd.properties objectForKey:GDTNativeAdDataKeyDesc])  {
        [self adapter:self didInterstitialFailAd:[AdsYuMIError errorWithCode:AdYuMIRequestNotAd description:@"GDT no ad"]];
    return;
  }
  ...
  if ([UIDevice currentDevice].orientation == UIDeviceOrientationPortrait || [UIDevice currentDevice].orientation == UIDeviceOrientationPortraitUpsideDown) {
    interstitialStr=[NSString stringWithFormat:interstitialStr,@"100%",@"100%",@"100%",@"100%",@"100%",@"70%",@"100%",@"100%",@"100%",@"50%",bigImg,@"100%",iconImg,title,desc,star,@"about:blank"];
  }else {
    interstitialStr = [NSString stringWithFormat:interstitialStr,@"100%",@"100%",@"50%",@"100%",@"100%",@"20%",@"2%",@"74%",@"100%",@"100%",@"65%",@"5%",@"30%",iconImg,title,star,bigImg,@"100%",desc,@"about:blank"];
  }
