...
- (void)requestAdWithIsPortrait:(BOOL)isPortrait isiPad:(BOOL)isiPad {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        strongSelf.webView.delegate = self;
        strongSelf.webView.scrollView.scrollEnabled = NO;
        // nativeAD init
        strongSelf.nativeAd =
        [[GDTNativeAd alloc] initWithAppkey:strongSelf.provider.data.key1 placementId:self.provider.data.key2];
        strongSelf.nativeAd.controller = strongSelf.delegate.rootViewControllerForPresentingBannerView;
        strongSelf.nativeAd.delegate = self;
...
- (void)nativeAdSuccessToLoad:(NSArray *)nativeAdDataArray {
    ...
    if (self.templateModel) {
        ...
        str = [self.templateManager replaceHtmlCharactersWithString:str iconURL:[_currentAd.properties objectForKey:GDTNativeAdDataKeyIconUrl] title:[_currentAd.properties objectForKey:GDTNativeAdDataKeyTitle] description:[_currentAd.properties objectForKey:GDTNativeAdDataKeyDesc] imageURL:@"大图" hyperlinkURL:@"跳转"];
    }
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    [self.nativeAd attachAd:_currentAd toView:webView];
