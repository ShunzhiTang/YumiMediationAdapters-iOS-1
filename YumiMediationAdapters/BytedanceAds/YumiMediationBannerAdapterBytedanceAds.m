...
- (void)requestAdWithIsPortrait:(BOOL)isPortrait isiPad:(BOOL)isiPad {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (weakSelf.isSmartBanner) {
            requestSize = [[YumiTool sharedTool] fetchBannerAdSizeWith:weakSelf.bannerSize smartBanner:weakSelf.isSmartBanner];
        ...
        weakSelf.bannerView = [[BUBannerAdView alloc] initWithSlotID:weakSelf.provider.data.key2 size:adSize rootViewController:[weakSelf.delegate rootViewControllerForPresentingModalView]];
