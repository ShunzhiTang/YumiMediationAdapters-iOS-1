...
@interface AdsYuMIAdNetworkNativeInterstitalGoogleAdapter()<GADNativeAppInstallAdLoaderDelegate,GADNativeAdDelegate> {
...
-(void)getAd{
  ...
  if ([_timeInterval isKindOfClass:[NSNumber class]]) {
    ...
    timer = [NSTimer scheduledTimerWithTimeInterval:15 target:self selector:@selector(timeOutTimer) userInfo:nil repeats:NO];
  }
  ...
  self.adLoader = [[GADAdLoader alloc] initWithAdUnitID:self.provider.key1 rootViewController:[self viewControllerForWillPresentInterstitialModalView]
                                                adTypes:adTypes
                                                options:@[option]];
...
-(void)timeOutTimer{
  [self stopTimer];
  [self adapter:self didInterstitialFailAd:[AdsYuMIError errorWithCode:AdYuMIRequestTimeOut description:@"Admob native time out"]];
}
...
- (void)adLoader:(GADAdLoader *)adLoader didFailToReceiveAdWithError:(GADRequestError *)error {
  ...
  isReading=YES;
  [self stopTimer];
  [self adapter:self didInterstitialFailAd:[AdsYuMIError errorWithCode:AdYuMIRequestNotAd description:@"Admob native no ad"]];
}
...
- (void)adLoader:(GADAdLoader *)adLoader didReceiveNativeAppInstallAd:(GADNativeAppInstallAd *)nativeAppInstallAd {
  ...
  if ([UIDevice currentDevice].orientation == UIDeviceOrientationPortrait || [UIDevice currentDevice].orientation == UIDeviceOrientationPortraitUpsideDown){
    appInstallAdView = [[NSBundle mainBundle] loadNibNamed:@"AdmobNativeInstallAdView" owner:nil options:nil].firstObject;
  }else {
    appInstallAdView = [[NSBundle mainBundle] loadNibNamed:@"AdmobNativeInstallAdView_Lan" owner:nil options:nil].firstObject;
  }
